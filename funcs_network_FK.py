...
def roads_type_folium(file_graphml, road_type, place_country):
    ...
    # my_map.save("Catania_partial.html")
    my_map.save("ROMA_OpenStreetMap_colors.html")
    return my_map
...
def centrality(file_graphml, place_country, bc=False, cc=False): #road_type
    ...
    if cc:
        ...
        DF_edge_centrality.to_csv("close_centrality.csv")  ## only one node..
    ...
    if bc:
        ...
        ## save centrality into a .csv file
        DF_edge_centrality.to_csv("btw_centrality_cost.csv")  ## only one node..
        ...
        DF_centrality.to_csv("btw_centrality_u_v_cost.csv")  ## only one node..
