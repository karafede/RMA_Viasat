...
'id'         =  ????
_____________________________Vaiabili da Aggiungere
    ...
    if Veic_da_Eliminare == 0:  # = 0 il veicolo viene ELABORATO
        ...
        if sum( dati1v.Fl_Viag>19  ) > 2:          #  ALMENO 2 VIAGGI FINITI
            ...
            try:
                ...
                df_Viag_1v.to_sql("route_FK_new", con=connection, schema="public",
                                   if_exists='append')
                connection.close()
...
try:
    connection = engine.connect()
    INFO_Veic.to_sql("idterm_Info_FK", con=connection, schema="public",
                      if_exists='append')
    connection.close()
except exc.OperationalError:
    print('OperationalError')
...
# df_Viag.to_csv('df_Viaggi.csv', sep=',')
tempo("==== Sta salvando il file 'INFO_Veic'      ", t0)
# INFO_Veic.to_csv('INFO_Veic.csv', sep=',')
df_Eliminati.to_csv('df_Eliminati.csv', sep=',')
