...
distance_border_to_border = great_circle_track_node(11.9214+ext, 41.56645+ext, 13.22569-ext, 42.13107-ext)
        ...
        if len(viasat) > 0:
            ...
            for i in range(len(diff_time)):
                if viasat_data.vehtype.iloc[0] == 1:
                    if (diff_time.iloc[i] >= 1800):  ## 30 min  ## 180 seconds (interval between two trips)  (or progressive == 0 and vehtype == 1)
                            # and (viasat_data.panel.iloc[i - 1] == 0 or viasat_data.panel.iloc[i] == 0)):
                        row.append(i)
                if viasat_data.vehtype.iloc[0] == 2:
                    if (diff_time.iloc[i] >= 1800):   ## 30 min
                            # and (viasat_data.panel.iloc[i - 1] == 0 or viasat_data.panel.iloc[i] == 0)):
                            ...
                            if (sum(VIASAT_TRIP['progressive']) > 0):
                              ...
                              ####### TRAVEL time > 10 min ###############################################
                                ...
                                if ( (first_point_lat <= Lat_Max)  and (first_point_lat >= Lat_Min) and (first_point_lon >= Lon_Min) and
                                        ...
                                        VIASAT_TRIP[VIASAT_TRIP.segment == max(VIASAT_TRIP.segment)][['panel']].iloc[0][0] == 1 and stoptime_s > 1800):  # and stoptime_s > 1800
                                    print("--------------------------> in_out ----------------")
                                    VIASAT_TRIP['border'] = "in_out"
                                # if (    ((first_point_lat <= Lat_Max) and (first_point_lat >= Lat_Max_Int)) and
                                ...
                                #    VIASAT_TRIP['border'] = "in_out"
                                    ...
                                    # 2) last point  far from the border (inside) and engine OFF and next stoptime > 30 min (1800 secs)
                                        ...
                                        (first_point_lon <= Lon_Max) and (last_point_lat <= Lat_Max_Int) and (last_point_lat >= Lat_Min_Int) and (last_point_lon >= Lon_Min_Int) and
                                        (last_point_lon <= Lon_Max_Int) and VIASAT_TRIP[VIASAT_TRIP.segment == max(VIASAT_TRIP.segment)][['panel']].iloc[0][0] == 0 and
                                        VIASAT_TRIP[VIASAT_TRIP.segment == min(VIASAT_TRIP.segment)][['panel']].iloc[0][0] == 1  and
                                ...
                                elif ( (first_point_lat <= Lat_Max_Int) and (first_point_lat >= Lat_Min_Int) and (first_point_lon >= Lon_Min_Int) and
                                        (first_point_lon <= Lon_Max_Int) and (last_point_lat <= Lat_Max) and (last_point_lat >= Lat_Min) and (last_point_lon >= Lon_Min) and
                                        (last_point_lon <= Lon_Max) and VIASAT_TRIP[VIASAT_TRIP.segment == max(VIASAT_TRIP.segment)][['panel']].iloc[0][0] == 1 and
                                ...
                                if (len(missing) > 0):
                                    ...
                                    if (VIASAT_TRIP[VIASAT_TRIP.segment == min(VIASAT_TRIP.segment)][['segment']].iloc[0][0] not in missing and
                                          ...
                                          or VIASAT_TRIP[VIASAT_TRIP.segment == max(VIASAT_TRIP.segment)][['progressive']].iloc[0][0] > distance_border_to_border
                                          or (VIASAT_TRIP[VIASAT_TRIP.segment == min(VIASAT_TRIP.segment)][['panel']].iloc[0][0]) == 1 ): ## and near the bordo
                                        VIASAT_TRIP['border'] = "in_out"

                                    elif (VIASAT_TRIP[VIASAT_TRIP.segment == min(VIASAT_TRIP.segment)][['segment']].iloc[0][0] in missing):
                                        VIASAT_TRIP['border'] = "border_in"
                                        # 1) first point inside the boreder and progressive > 0 and engine ON
                                        # 2) last point  far from the border (inside) and engine OFF and next stop > 30 min
                                    elif (VIASAT_TRIP[VIASAT_TRIP.segment == max(VIASAT_TRIP.segment)][['segment']].iloc[0][0] in missing):
                                        VIASAT_TRIP['border'] = "border_out"

                                elif (len(missing) == 0):
                                    if (VIASAT_TRIP[VIASAT_TRIP.segment == max(VIASAT_TRIP.segment)][['panel']].iloc[0][0] == 1):
                                        VIASAT_TRIP['border'] = "border_out"   ## and near the border
                                        # dentro il bordo + motore acceso
                                    elif (VIASAT_TRIP[VIASAT_TRIP.segment == min(VIASAT_TRIP.segment)][['panel']].iloc[0][0] == 1
                                          and VIASAT_TRIP[VIASAT_TRIP.segment == min(VIASAT_TRIP.segment)][['progressive']].iloc[0][0] >= distance_border_to_border/2):
                                        VIASAT_TRIP['border'] = "border_in"
                                    else:
                                        VIASAT_TRIP['border'] = "in"
                                    # elif (VIASAT_TRIP[VIASAT_TRIP.segment == min(VIASAT_TRIP.segment)][['panel']].iloc[0][0] == 1):
                                ...
                                try:
                                    ...
                                    VIASAT_TRIP.to_sql("routecheck_trenta_bis", con=connection, schema="public",
                                                       if_exists='append')
                                    # VIASAT_TRIP.to_sql("routecheck_5912730", con=connection, schema="public",
                                    #                   if_exists='append')
